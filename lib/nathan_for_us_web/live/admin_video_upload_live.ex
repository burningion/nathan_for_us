defmodule NathanForUsWeb.AdminVideoUploadLive do
    ...
    case result do
      {:ok, video} ->
        socket =
          ...
          |> assign(:success_message, "Video '#{video.title}' uploaded and processed successfully!")
          |> assign(:error_message, nil)
          |> reset_form()
  ...
  defp process_video_upload(socket, title, description) do
    try do
      ...
      case VideoProcessor.process_video_with_captions(video.id, video_file_path, caption_file_path) do
        {:ok, _result} ->
          ...
          <%= if @processing_status do %>
            <div class="bg-blue-600 text-white p-4 rounded-lg mb-6">
              ...
              <div class="w-full bg-blue-800 rounded-full h-2">
                <div class="bg-blue-400 h-2 rounded-full transition-all duration-300" style={"width: #{@processing_progress}%"}></div>
              </div>
          ...
          <form phx-submit="upload" phx-change="validate" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-6">
              ...
              <div class="bg-gray-800 p-6 rounded-lg">
                ...
                >
                  <svg class="mx-auto h-12 w-12 text-gray-400 mb-4" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                    <path d="M9 12h6m6 0h6m-6 6h6m-12 6h12M9 24h6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>

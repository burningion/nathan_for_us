  ...
  describe "get_sample_caption_suggestions/1" do
    setup do
      ...
      test_captions = [
        ...
        "Very short",
        # Too long (> 80 chars)
        "This is an extremely long caption that exceeds the 80 character limit and should be filtered out completely from the results",
        # Empty
        "",
        # Only whitespace
        "   "
      ...
      captions =
        for {text, index} <- Enum.with_index(test_captions) do
          %{
            text: text,
    ...
    test "prioritizes business/nathan-related captions" do
      ...
      has_priority_content =
        Enum.any?(result, fn suggestion ->
    ...
    test "handles empty database gracefully" do
      ...
      assert is_list(result)
      # Changed from > 0 to >= 0 since empty DB may return fallback or empty

defmodule NathanForUsWeb.AdminFrameBrowserLiveTest do
  ...
  setup do
    ...
    regular_user = %{regular_user | confirmed_at: DateTime.utc_now() |> DateTime.truncate(:second)}
  ...
  describe "admin access" do
    ...
    test "regular user cannot access admin frame browser", %{conn: conn, regular_user: regular_user} do
      conn = log_in_user(conn, regular_user)
      
      # Should redirect with error
      assert {:error, {:redirect, %{to: "/", flash: %{"error" => "Access denied. Administrator privileges required."}}}} = 
  ...
  describe "video selection" do
    ...
    test "selecting a video loads frames", %{conn: conn, admin_user: admin_user, video: video} do
      ...
      html = render(view)
      assert html =~ video.title  # Check if video title appears anywhere
    end
  ...
  describe "frame selection" do
    test "can select and deselect individual frames", %{conn: conn, admin_user: admin_user, video: video} do
      conn = log_in_user(conn, admin_user)
  ...
  describe "GIF generation" do
    test "shows error when no frames selected", %{conn: conn, admin_user: admin_user, video: video} do
      conn = log_in_user(conn, admin_user)
      {:ok, view, _html} = live(conn, "/admin/frames?video_id=#{video.id}")
    ...
    test "starts GIF generation when frames are selected", %{conn: conn, admin_user: admin_user, video: video} do
      conn = log_in_user(conn, admin_user)
      {:ok, view, _html} = live(conn, "/admin/frames?video_id=#{video.id}")
  ...
  describe "error handling" do
    test "handles invalid frame index gracefully", %{conn: conn, admin_user: admin_user, video: video} do
      conn = log_in_user(conn, admin_user)
      {:ok, view, _html} = live(conn, "/admin/frames?video_id=#{video.id}")
